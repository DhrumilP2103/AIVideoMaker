...
protocol ProfileServiceProto {
    ...
    func updateProfile(name: String, completion: @escaping (Result<ProfileUpdateResponseModel, APIError>) -> Void)
    func logout(completion: @escaping (Result<GlobalResponseModel, APIError>) -> Void)
}
...
struct ProfileAPIService: ProfileServiceProto {
    ...
    func updateProfile(name: String, completion: @escaping (Result<ProfileUpdateResponseModel, APIError>) -> Void) {
        ...
        APIService().postAPI(modelType: ProfileUpdateResponseModel.self, endPoint: APIConstants.Endpoints.profileUpdate, parameter: parameter) { data, statusCode, success in
            switch statusCode {
            case 200:
                if let response = data {
                    completion(.success(response))
    ...
    func logout(completion: @escaping (Result<GlobalResponseModel, APIError>) -> Void) {
        ...
        APIService().getAPI(modelType: GlobalResponseModel.self, endPoint: APIConstants.Endpoints.logout, parameter: parameter) { data, statusCode, success in
            switch statusCode {
            case 200:
                if let response = data {
                    completion(.success(response))
            ...
            default:
                if let response = data {
                    ...
                    completion(.failure(.unexpected(NSLocalizedString("Logout failed. Please try again.", comment: ""))))
                }
            }
        }
    }
